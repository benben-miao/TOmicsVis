...
network_plot <- function(data,
	...
	if (calc_by == "degree") {
		...
		vcolor[deg > degree_value] <- colorRampPalette(c(from_color, to_color))(10)
		vertex_shape <- rep(normal_shape, igraph::vcount(net))
		vertex_shape[deg > degree_value] <- spatial_shape
	} else if (calc_by == "node") {
		vcolor <- rep(normal_color, igraph::vcount(net))
		vcolor[V(net)$name %in% data$node1] <- colorRampPalette(c(from_color, to_color))(10)
		vertex_shape <- rep(normal_shape, igraph::vcount(net))
	...
	return(
		igraph::plot.igraph(
			...
			layout = do.call(net_layout, list(net)),
			edge.arrow.mode = 1,
			edge.arrow.size = 0.0,
